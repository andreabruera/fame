...
class CurrentSetup:
    ...
    def __init__(self, entity_type, args):
        ...
        self.analysis = 'transfer_decoding' 
...
for entity_type in entity_types:
    ...
    #for layer in layers:
    models = list(set([w.split('_')[0] for w in os.listdir(folder)]))
    models = [k for k in models if k!='people' and k!='places']
    models_list = models.copy()
    models = ['transe', 'w2v', 'wikipedia2vec', 'elmo', 'bert', 'ernie']
    assert set(models) == set(models_list)
    ...
    for model_index, data in enumerate(plot_data):
        '''
        #layer = layer + 1 if args.model=='bert' else ''
        ax[0].bar(xs[model_index]-.5, numpy.average(data), \
                  color=colors[model_index], alpha=0.2, edgecolor='black', \
                  width=0.2)
        ax[0].scatter([xs[model_index]-.5 for i in data], data, s=3.)
        '''
        ax[0].violinplot(data, \
                         positions=[model_index+.5], \
                         showmedians=True)
    #color=colors[model_index]) 

    ax[0].hlines(y=0.5, xmin=0., xmax=len(xs), \
                 linestyle='dashed', color='gray')
    ax[0].set_xticks([x-.5 for x in xs])
    ax[0].set_xticklabels(models)
    ax[0].set_title('Decoding for {}, using {} - '\
                    '{} evaluation'.format(\
    ...
    for model_index, data in enumerate(plot_data):
        ...
        #layer = layer + 1 if args.model=='bert' else ''
        if model_index < len(plot_data)/2:
            x_text = .5
            y_text = model_index+.5
        else:
            x_text = (len(xs)/2)+.5
            y_text = (model_index-(len(plot_data)/2))+.5
        #x_text = .5
        #y_text = len(plot_data)-model_index

        label='model {} - avg {} - median {}'.format(models[model_index], \
    ...
    matrix_path = os.path.join(plot_path, \
               'confusion_{}_{}_{}.png'.format(\
               args.analysis, args.evaluation_method, \
               entity_type))
    confusion_matrix_simple(matrix, models,matrix_title, matrix_path)
