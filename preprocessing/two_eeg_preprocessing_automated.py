...
def preprocess_eeg(s):
    ...
    for r in range(1, 25):
        ...
        ### Events
        run_events = mne.find_events(full_raw)
        if s == 2:
            ev_n = r -1
        else:
            ev_n = r
        eve_filename = 'sub-{:02}_run-{:02}.events'.format(s, ev_n)
        eve_path = os.path.join(main_folder, events_folder, eve_filename)
        with open(eve_path) as i:
            triggers = [l.strip().split('\t')[1] for l in i.readlines()]
        if s != 1:
            triggers = [int(t) for t in triggers[1:]]
        ### Fixes for event files in subject  1
        else:
            triggers = [int(t) for t in triggers]
        if s == 1 and run_events.shape[0] == 31:
            triggers = [t for t in triggers if t != 1]
            for t_i, t in enumerate(triggers):
                run_events[t_i, 2] = t
        run_events = run_events[run_events[:, 2]<200]

        assert run_events.shape[0] <= 32
        if run_events.shape[0] != 32:
...
main_folder = '/import/cogsci/andrea/dataset/neuroscience/FamNamesEEG/raw_data'

montage = mne.channels.make_standard_montage(kind='biosemi128')
with Pool(processes=os.cpu_count()-1) as pool:
    problems = pool.map(preprocess_eeg, [i for i in range(33, 34)])
