...
def preprocess_eeg(s):
    ...
    for r in range(1, 24+1):
        ...
        ### Loading events, so as to be able to correct them for subject 1
        ...
        ### Fixing wrong events
        mask = run_events_full[:, 2] < 120
        run_events = run_events_full[mask]
        assert len(run_events) == len(current_lines)

        if s == 1:
            for l, e in zip(current_lines, run_events):
                e[2] = int(l[3])
            full_raw.add_events(run_events, replace=True, stim_channel='Status')
        ...
        if run_events.shape[0] > 3:
            ...
            run_epochs.apply_baseline(baseline=(None, 0))
            ### Autoreject EEG channels and epochs
...
subjects = [i for i in range(1, 34)]
